...
fn decay_needs(clock: Res<SimulationClock>, mut needs_query: Query<&mut Needs>) {
    let delta_days = clock.delta_days();

    for mut needs in &mut needs_query {
        needs.hunger = (needs.hunger + delta_days * 0.05).min(1.0);
